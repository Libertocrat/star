...
Modules shipped under `src/star/actions/specs` are loader-derived `CORE` modules. Modules mounted through the deployment `user-specs/` directory are `EXTENSION` modules. Every action must match an exact reviewed invocation form authorized for its provenance. Extensions must additionally declare reviewed capabilities, remain within the enabled `STAR_ENABLED_EXTENSION_CAPABILITIES` selection, and match a form that grants the declared capability; CORE forms require no extension capability.
