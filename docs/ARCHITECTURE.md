...
`GET /v1/files` treats pagination cursors as bounded opaque storage tokens. `core/files/listing.py` owns the canonical query, local versioned cursor codec, query fingerprint, deterministic ordering position, and pure pagination transforms; the route handler converts typed boundary input into that query and the active storage adapter validates and interprets its cursor token. The endpoint applies exact `status`, `mime_type`, and `extension` filters, an ASCII case-insensitive substring filter only over editable `file_name`, and an all-of CSV `tags` filter. Tags normalize through the same lowercase grammar and limits used by metadata replacement. Local cursors carry a backend discriminator, `(created_at, id)` position, and SHA-256 fingerprint over the effective `sort`, `order`, `status`, `mime_type`, `extension`, `file_name`, and canonical tag context. Changing any of those values invalidates continuation, while changing `limit` remains valid. The route rejects unknown, repeated, and empty query parameters before listing. Tokens are limited to 4096 characters, reject malformed, ambiguous, legacy, unsupported-version, backend-mismatched, and context-mismatched payloads as `400 INVALID_REQUEST`, and are never included verbatim in logs. Cursors do not provide snapshot isolation: concurrent file creation, update, or deletion may affect later pages. The backend-facing protocol receives the canonical query while keeping continuation state backend-owned, so a future S3/MinIO adapter can use different opaque cursor state without changing the HTTP handler.
...
- limits the application service and its subprocesses to 256 PIDs plus operator-configurable `STAR_CONTAINER_MEMORY_LIMIT` and `STAR_CONTAINER_CPUS_LIMIT` hard caps, defaulting to `1g` and `1.0`
