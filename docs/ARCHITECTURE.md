...
`GET /v1/files` treats pagination cursors as bounded opaque storage tokens. `core/files/listing.py` owns the local versioned cursor codec, canonical query fingerprint, deterministic ordering position, and pure pagination transforms; the route handler passes the token unchanged to `ManagedFileStore`, and the active storage adapter validates and interprets it. Local cursors carry a backend discriminator, `(created_at, id)` position, and SHA-256 fingerprint over the effective `sort`, `order`, `status`, `mime_type`, and `extension` context. Changing any of those values invalidates continuation, while changing `limit` remains valid. Tokens are limited to 4096 characters, reject malformed, ambiguous, legacy, unsupported-version, backend-mismatched, and context-mismatched payloads as `400 INVALID_REQUEST`, and are never included verbatim in logs. Cursors do not provide snapshot isolation: concurrent file creation, update, or deletion may affect later pages. The backend-facing protocol exposes only the opaque token so a future S3/MinIO adapter can own different continuation state without changing the HTTP handler.
