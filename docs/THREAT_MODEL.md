...
- provenance-scoped exact invocation-policy checks for every action at build time and immediately before process creation, plus extension capability admission and independent runtime binary policy checks
...
- Provenance-scoped per-binary invocation forms constrain which values can occupy each command position for both built-in and mounted modules. Extension-visible forms additionally require reviewed, operator-enabled capabilities. The selected authorization, form, and permitted token origins are compiled into immutable action policy rather than trusted from YAML at runtime.
...
- `build_actions()` compiles only validated modules into immutable runtime `ActionSpec` objects, preserving loader-derived provenance and requiring the selected invocation form and authorization.
...
- After action-parameter validation and before rendering, runtime applies request-dependent option requirements from the compiled form for every provenance. A missing required option or required-any-of selection is returned as safe `INVALID_PARAMS` without creating invocation-owned resources. Immediately before process creation, a separate verifier treats inconsistent provenance authorization, rendered token form, option invariants, value bounds, expansion counts, and managed ownership as internal integrity failures; it never logs raw argv or paths and triggers dispatcher cleanup. If a managed input becomes unavailable only in that final interval, STAR returns safe `INVALID_PARAMS` rather than exposing storage details.
- `execute_command()` independently rejects binary paths, compiled-binary mismatches, blocked binaries, and non-allowlisted binaries before subprocess execution, applies configured runtime timeouts, and cleans the owned process group on timeout or cancellation where supported.
