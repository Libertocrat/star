...
def encode_cursor(metadata: FileMetadata) -> str:
    """Encode an opaque cursor from metadata identity and timestamp.

    Args:
        metadata: Metadata record whose stable ordering key should become the
            next-page cursor.

    Returns:
        URL-safe base64 cursor containing the record `created_at` and `id`.
    """
...
def decode_cursor(cursor: str) -> tuple[datetime, UUID]:
    """Decode an opaque file-list cursor into its ordering key.

    Args:
        cursor: URL-safe base64 cursor previously returned by `encode_cursor`.

    Returns:
        Tuple of creation timestamp and file UUID used for deterministic paging.

    Raises:
        ValueError: If the cursor cannot be decoded into a valid timestamp or UUID.
        KeyError: If the decoded cursor payload is missing required keys.
...
def apply_pagination(
    ...
    *,
    ...
    Args:
        ...
        cursor: Optional decoded cursor key from the previous page.
