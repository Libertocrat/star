...
# STAR container. This allows accessing the containerized STAR service from the
...
#
# Optional overrides:
#   --container <name>   STAR container name (default: auto-detect)
...
Description:
  ...
  This allows accessing the containerized STAR service without exposing
...
Examples:
  ...
  # Forward to a specific STAR container
  $SCRIPT_NAME --env-file .env --container star-1
  ...
  export STAR_SHARED_NETWORK=docker-network
  export STAR_PORT=8080
...
# -----------------------------------------------------------------------------
# Resolve STAR container
# -----------------------------------------------------------------------------

STAR_CONTAINER="$(resolve_star_container)"

log "Resolved STAR container: $STAR_CONTAINER"
...
log "STAR container : $STAR_CONTAINER"
log "STAR port      : $STAR_PORT"
log "Local port    : $LOCAL_PORT"
log "Docker network: $STAR_SHARED_NETWORK"

echo
log "Forwarding http://localhost:${LOCAL_PORT} -> ${STAR_CONTAINER}:${STAR_PORT}"
