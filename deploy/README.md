...
Install [Cosign](https://docs.sigstore.dev/cosign/system_config/installation/) before verifying release origin. Select a release tag, verify the signed checksum manifest, and then verify downloaded release assets against it:

```bash
VERSION=vX.Y.Z
BASE_URL="https://github.com/Libertocrat/star/releases/download/${VERSION}"
IDENTITY="https://github.com/Libertocrat/star/.github/workflows/release.yml@refs/tags/${VERSION}"
...
Release images are signed. Verify the image before deployment:

```bash
IMAGE="ghcr.io/libertocrat/star:${VERSION}"
...
Tags are convenient selectors; use the resolved `@sha256:...` image reference for immutable deployment policy.
...
The runtime root filesystem remains writable because STAR has not yet proven every required temporary write path under an explicit volume or tmpfs. The capability, privilege, PID, and shutdown controls introduced in `0.1.4` remain in place.
