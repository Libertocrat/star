...
# Track a STAR-managed file id once, preserving a role label for summaries.
track_created_file() {
    local file_id="${1-}"
...
# Execute a STAR action using the concise request body with only params.
execute_action() {
    ...
    if ! star_api_post_json "/v1/actions/${action_id}" "${request_json}"; then
        return 1
    fi

    if ! star_api_require_success "Execute action ${action_id}"; then
    ...
    DEMO_LAST_ACTION_STDOUT="$(jq -r '.data.stdout // ""' <<< "${STAR_API_LAST_BODY}")"
    DEMO_LAST_EXIT_CODE="$(jq -r '.data.exit_code // "n/a"' <<< "${STAR_API_LAST_BODY}")"
    DEMO_LAST_EXEC_TIME="$(jq -r '.data.exec_time // "n/a"' <<< "${STAR_API_LAST_BODY}")"
    DEMO_LAST_ACTION_BODY="${STAR_API_LAST_BODY}"
    return 0
}
...
upload_demo_asset() {
    ...
    if ! star_api_upload_file "${DEMO_ASSET_FILE}"; then
        return 1
    fi

    if ! star_api_require_success "Upload demo file"; then
        return 1
    fi

    file_id="$(star_api_extract_file_id_from_upload)"
    filename="$(star_api_extract_metadata_field "original_filename")"
...
cleanup_created_files() {
    ...
    if [[ "${KEEP_FILES:-false}" == "true" ]]; then
        info "Keeping STAR-managed files created by this demo."
    ...
    for file_id in "${CREATED_FILE_IDS[@]}"; do
        ...
        if star_api_delete_file "${file_id}" "true" && [[ "${STAR_API_LAST_STATUS}" =~ ^2[0-9]{2}$ ]] && jq -e '.success == true' >/dev/null 2>&1 <<< "${STAR_API_LAST_BODY}"; then
            ...
            warn "Failed to delete STAR-managed file ${file_id} (role: ${role_label}, HTTP ${STAR_API_LAST_STATUS})."
        fi
    done
