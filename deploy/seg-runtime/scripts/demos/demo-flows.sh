...
if ! command -v reset_demo_state >/dev/null 2>&1; then
    ...
    source "${SEG_SCRIPTS_DIR}/demos/demo-common.sh"
fi
...
run_demo_files() {
    ...
    seg_api_get "/v1/files/${file_id}" || return 1
    seg_api_require_success "Get file metadata" || return 1

    printf '\nResult:\n'
    printf '  %-27s %s\n' "file_id" "$(seg_api_extract_metadata_field "id")"
    printf '  %-27s %s\n' "original_filename" "$(seg_api_extract_metadata_field "original_filename")"
    ...
    seg_api_get "/v1/files?limit=5&order=desc" || return 1
    seg_api_require_success "List files" || return 1

    listed_count="$(jq -r '.data.pagination.count // 0' <<< "${SEG_API_LAST_BODY}")"
    mapfile -t listed_items < <(jq -r '.data.files[]? | "\(.id)  \(.original_filename)"' <<< "${SEG_API_LAST_BODY}")
...
run_demo_actions() {
    ...
    seg_api_get "/v1/actions" || return 1
    seg_api_require_success "List actions" || return 1

    module_count="$(jq -r '.data.modules | length' <<< "${SEG_API_LAST_BODY}")"
    action_count="$(jq -r '[.data.modules[]?.actions[]?] | length' <<< "${SEG_API_LAST_BODY}")"
    mapfile -t selected_modules < <(jq -r '[.data.modules[]?.module_id] | unique | .[:8] | .[]?' <<< "${SEG_API_LAST_BODY}")
    ...
    seg_api_get "/v1/actions?tags=crypto" || return 1
    seg_api_require_success "List crypto actions" || return 1
    mapfile -t crypto_actions < <(jq -r '[.data.modules[]?.actions[]? | {id:.action_id,summary:(.summary // ""),tags:(.tags // [])}] | unique_by(.id) | .[]? | @base64' <<< "${SEG_API_LAST_BODY}")
    ...
    seg_api_get "/v1/actions/base.crypto.encrypt_file_aes256" || return 1
    if ! seg_api_require_success "Get action spec base.crypto.encrypt_file_aes256"; then
    ...
    tags_joined="$(jq -r '.data.tags | join(",") // ""' <<< "${SEG_API_LAST_BODY}")"
    mapfile -t arg_rows < <(jq -r '.data.args[]? | "name=\(.name // "-") type=\(.type // "-") required=\(.required // false)"' <<< "${SEG_API_LAST_BODY}")
    mapfile -t output_rows < <(jq -r '.data.outputs[]? | "name=\(.name // "-") type=\(.type // "-")"' <<< "${SEG_API_LAST_BODY}")

    printf '\nResult:\n'
    printf '  %-27s %s\n' "action_id" "$(jq -r '.data.action_id // ""' <<< "${SEG_API_LAST_BODY}")"
    printf '  %-27s %s\n' "summary" "$(jq -r '.data.summary // ""' <<< "${SEG_API_LAST_BODY}")"
    printf '  %-27s %s\n' "tags" "${tags_joined}"
...
run_demo_inspect() {
    ...
    printf 'What this demo shows:\n'
    printf '  Read-only analysis actions over SEG file IDs instead of local paths.\n\n'
...
run_demo_encrypt() {
    ...
    original_sha="$(seg_api_extract_metadata_field "sha256")"
    original_mime="$(seg_api_extract_metadata_field "mime_type")"
    original_size="$(seg_api_extract_metadata_field "size_bytes")"
    demo_pause
    ...
    encrypted_id="$(seg_api_extract_output_file_field "encrypted_file" "id")"
    encrypted_sha="$(seg_api_extract_output_file_field "encrypted_file" "sha256")"
    encrypted_mime="$(seg_api_extract_output_file_field "encrypted_file" "mime_type")"
    encrypted_size="$(seg_api_extract_output_file_field "encrypted_file" "size_bytes")"
    ...
    decrypted_id="$(seg_api_extract_output_file_field "decrypted_file" "id")"
    decrypted_sha="$(seg_api_extract_output_file_field "decrypted_file" "sha256")"
    decrypted_mime="$(seg_api_extract_output_file_field "decrypted_file" "mime_type")"
    decrypted_size="$(seg_api_extract_output_file_field "decrypted_file" "size_bytes")"
